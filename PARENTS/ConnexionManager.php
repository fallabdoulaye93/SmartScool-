...
class ConnexionManager
{
    public function Connecter($login, $motpass)
    {
        ...
        $lib = new Librairie();
        $query = "SELECT IDINDIVIDU, MATRICULE, NOM, PRENOMS, IDTYPEINDIVIDU, INDIVIDU.IDETABLISSEMENT, ETABLISSEMENT.PREFIXE,ETABLISSEMENT.LOGO, ETABLISSEMENT.NOMETABLISSEMENT_ 
                  FROM INDIVIDU, ETABLISSEMENT ";
                  $query .= " WHERE ETABLISSEMENT.IDETABLISSEMENT = INDIVIDU.IDETABLISSEMENT 
                  AND INDIVIDU.IDTYPEINDIVIDU = 9 AND LOGIN = " . $lib->GetSQLValueString($_POST['login'], "text") . " 
