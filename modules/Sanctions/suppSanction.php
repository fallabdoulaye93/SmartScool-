<?php
session_start();
require_once("../../restriction.php");
require_once("../../config/Connexion.php");
require_once ("../../config/Librairie.php");
$connection =  new Connexion();
$dbh = $connection->Connection();
$lib =  new Librairie();

if($_SESSION['profil']!=1) 
$lib->Restreindre($lib->Est_autoriser(46,$_SESSION['profil']));


require_once("classe/SanctionManager.php");
require_once("classe/Sanction.php");
$sanction=new SanctionManager($dbh,'SANCTION');

$colname_rq_annee_etab = "-1";
if (isset($_GET['IDSANCTION']))
{
    $colname_rq_annee_etab = $lib->securite_xss($_GET['IDSANCTION']);
}

$res = $sanction->supprimer('IDSANCTION',$colname_rq_annee_etab);

if ($res==1)
{
    $msg="suppression reussie";
}
else
{
    $msg="suppression echouee";
}
header ("Location: accueil.php?msg=".$lib->securite_xss($msg)."&res=".$lib->securite_xss($res));

?>