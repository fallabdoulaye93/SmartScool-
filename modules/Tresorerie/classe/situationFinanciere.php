...
$lib->Restreindre($lib->Est_autoriser(31,$_SESSION['profil']));


$colname_rq_mois = "-1";
if (isset($_SESSION['etab'])) {
  $colname_rq_mois = $_SESSION['etab'];
}
$colname1_rq_mois = "-1";
...
$query_rq_mois = $dbh->query("SELECT MONTH(MENSUALITE.DATEREGLMT) as mois, INSCRIPTION.IDINSCRIPTION, FACTURE.NUMFACTURE FROM MENSUALITE, INSCRIPTION, FACTURE WHERE MENSUALITE.IDETABLISSEMENT = ".$colname_rq_mois."  AND FACTURE.IDINSCRIPTION=INSCRIPTION.IDINSCRIPTION AND  INSCRIPTION.IDANNEESSCOLAIRE=".$colname1_rq_mois."  GROUP BY MONTH(MENSUALITE.DATEREGLMT)");
...
<?php include('header.php'); ?>
                ...
                <ul class="breadcrumb">
                    ...
                    <li class="active">Situation Financi&egrave;re</li>
                </ul>
                <!-- END BREADCRUMB -->                       
	...
	//TOTAL SCOLARITE
  		$colname_rq_scolarite = $_SESSION['etab'];
		$coldate_rq_scolarite = $row_rq_mois['mois'];
		$colname1_rq_scolarite = $_SESSION['ANNEESSCOLAIRE'];
		
		
		$query_rq_scolarite = $dbh->query("SELECT SUM(MENSUALITE.MONTANT) as somme_verse, INSCRIPTION.IDINSCRIPTION FROM MENSUALITE,FACTURE, INSCRIPTION WHERE  MENSUALITE.IDETABLISSEMENT = ".$colname_rq_scolarite."  AND  FACTURE.IDINSCRIPTION=INSCRIPTION.IDINSCRIPTION AND MONTH(MENSUALITE.DATEREGLMT)=".$coldate_rq_scolarite."  AND INSCRIPTION.IDANNEESSCOLAIRE=".$colname1_rq_scolarite);
		...
		$row_rq_scolarite = $query_rq_scolarite->fetchObject();
			...
			$col_etab_rq_somme_prof = $_SESSION['etab'];
			$col_anne_rq_somme_prof = $_SESSION['ANNEESSCOLAIRE'];
			
			$query_rq_somme_prof = $dbh->query("SELECT SUM(REGLEMENT_PROF.MONTANT) AS SOMME_PROF, RECRUTE_PROF.IDRECRUTE_PROF FROM REGLEMENT_PROF, RECRUTE_PROF WHERE  MONTH(REGLEMENT_PROF.DATE_REGLEMENT )= ".$colname_rq_somme_prof." AND REGLEMENT_PROF.INDIVIDU=RECRUTE_PROF.IDINDIVIDU AND RECRUTE_PROF.IDETABLISSEMENT=".$col_etab_rq_somme_prof." AND RECRUTE_PROF.IDANNEESSCOLAIRE=".$col_anne_rq_somme_prof);
 ...
 // AUTRES CHARGES
		...
		$col_ann_rq_equipement = $_SESSION['ANNEESSCOLAIRE'];

		
		$query_rq_equipement = $dbh->query("SELECT SUM(REGLEMENT_PERSO.MONTANT) AS SOMME_PERSO FROM REGLEMENT_PERSO, INDIVIDU WHERE  MONTH(REGLEMENT_PERSO.DATE_REGLEMENT )= ".$col_mois_rq_equipement." AND REGLEMENT_PERSO.INDIVIDU=INDIVIDU.IDINDIVIDU AND  REGLEMENT_PERSO.IDANNEESCOLAIRE=".$col_ann_rq_equipement);
		...
		$col_ann_rq_depense = $_SESSION['ANNEESSCOLAIRE'];
