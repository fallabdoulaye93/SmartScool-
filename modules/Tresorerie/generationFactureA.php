...
$query_rq_classe = $dbh->query("SELECT `IDCLASSROOM`, `LIBELLE`, `IDNIVEAU`, `IDETABLISSEMENT`, `IDSERIE` FROM `CLASSROOM` WHERE IDETABLISSEMENT = " . $colname_rq_classe);
if (isset($_POST['envoyer']) && $_POST['envoyer'] != "" && $_POST['MOIS'] != "" && $_POST['ANNEE'] != "") {
    ...
    $query_rq_inscription = $dbh->query("SELECT `IDINSCRIPTION`, `DATEINSCRIPT`, `FRAIS_INSCRIPTION`, `MONTANT`, `ACCOMPTE_VERSE`, `STATUT`, `IDNIVEAU`, `IDINDIVIDU`, `IDETABLISSEMENT`, `IDANNEESSCOLAIRE`, `IDSERIE`, `DERNIER_ETAB`, `VALIDETUDE`, `FRAIS_DOSSIER`, `FRAIS_EXAMEN`, `UNIFORME`, `VACCINATION`, `ASSURANCE`, `FRAIS_SOUTENANCE`, `TRANSPORT`, `MONTANT_TRANSPORT`, `MONTANT_DOSSIER`,
                                                          `MONTANT_EXAMEN`, `MONTANT_UNIFORME`, `MONTANT_VACCINATION`, `MONTANT_ASSURANCE`, `MONTANT_SOUTENANCE`, `RESULTAT_ANNUEL`, `ACCORD_MENSUELITE` FROM `INSCRIPTION` WHERE IDETABLISSEMENT = " . $colname_rq_inscription . " AND IDANNEESSCOLAIRE=" . $lib->securite_xss($_SESSION['ANNEESSCOLAIRE']));
    ...
    if ($query_rq_inscription->fetchColumn() > 0) {
        foreach ($query_rq_inscription->fetchAll() as $row_rq_inscription) {
            $lib->Generer_facture($mois, $row_rq_inscription['ACCORD_MENSUELITE'], $row_rq_inscription['IDINSCRIPTION'], $lib->securite_xss($_SESSION['etab']));
        }
        $res = 1;
            ...
            <div class="panel-body">
                ...
                <form id="form1" name="form1" method="post" action="">
                    <fieldset class="cadre">
                        ...
                        <div class="row" onmouseout=" $('#MOIS').removeAttr('required');$('#ANNEE').removeAttr('required');">
                            <div class="col-lg-12">
                                <div class="col-lg-4">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        <div onmouseover="$('#MOIS').attr('required','required');$('#ANNEE').attr('required','required');">
                ...
                <br><br>
                <form id="form" name="form" method="post" onsubmit="if($('#Classe').val() == 'null'){$('button.selectpicker').css('border-color','#E04B4A');$('#Classe-error').removeClass('hidden'); return false;} " action="factureIndividuelle.php">
                    <fieldset class="cadre">
                        <legend> G&eacute;n&eacute;ration facture individuelle</legend>
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="col-lg-6">
                                    <div class="col-lg-3"><label class="control-label">CLASSE</label></div>
                                    <div class="col-lg-9">
                                        <select onchange="$('button.selectpicker').css('border-color','#DDD');$('#Classe-error').addClass('hidden');" name="Classe" id="Classe" class="form-control selectpicker"
                                                data-live-search="true">
                                            <option value="null">--S&eacute;lectionner--</option>
                                            <?php foreach ($query_rq_classe->fetchAll() as $row_rq_classe) { ?>
                                                <option
                                                    value="<?php echo $row_rq_classe['IDCLASSROOM']; ?>"><?php echo $row_rq_classe['LIBELLE']; ?>
                                                </option>
                                            <?php } ?>
                                        </select>
                                ...
                                <div class="col-lg-1">
                                    <div class="form-group">
                                        <div>
                                            <input type="submit" class="btn btn-success" name="envoyerFactInd"
                                                   value="Valider"/>
                                        </div>
                                    </div>
                                </div>
