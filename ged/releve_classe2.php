...
if (isset($_GET['idclasse'])) {
  $colname_rq_classe_eleve = $_GET['idclasse'];
}
...
$query_rq_classe_eleve = $dbh->query("SELECT IDINDIVIDU FROM AFFECTATION_ELEVE_CLASSE WHERE IDCLASSROOM = ".$colname_rq_classe_eleve." AND AFFECTATION_ELEVE_CLASSE.IDANNEESSCOLAIRE=".$colname2_rq_classe_eleve);
...
$posY=10;
foreach($query_rq_classe_eleve->fetchAll() as $row_rq_classe_eleve)
{
	$colname_rq_eleve =$row_rq_classe_eleve['IDINDIVIDU'];
	if (isset($_GET['id_eleve'])) {
	  $colname_rq_eleve = $_GET['id_eleve'];
	}
	...
	$nb_cresh_obt=0;
	$moy_sem=0;
	foreach($query_rq_ue->fetchAll() as $row_rq_ue)
	{
		$idue= $row_rq_ue['IDUE'];
		...
		foreach($query_rq_matiere->fetchAll() as $row_rq_matiere )
		{
			...
			$query_rq_cc = $dbh->query("SELECT avg( NOTE.NOTE) as moy FROM NOTE, CONTROLE WHERE NOTE.IDCONTROLE=CONTROLE.IDCONTROLE AND CONTROLE.IDTYP_CONTROL=5 AND CONTROLE.IDMATIERE=".$colname_rq_cc." AND NOTE.IDINDIVIDU=".$colname2_rq_cc." AND CONTROLE.IDPERIODE=".$colname3_rq_cc." AND VALIDER='oui'");
...
$pdf->Output('BULLETINS_NOTES.pdf', 'D');
