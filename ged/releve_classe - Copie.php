...
mysql_select_db($database_connexion, $connexion);
$query_rq_classe_eleve = sprintf("SELECT IDINDIVIDU FROM AFFECTATION_ELEVE_CLASSE WHERE IDCLASSROOM = %s AND AFFECTATION_ELEVE_CLASSE.IDANNEESSCOLAIRE=%s", GetSQLValueString($colname_rq_classe_eleve, "int"),GetSQLValueString($colname2_rq_classe_eleve, "int"));
...
while($row_rq_classe_eleve = mysql_fetch_assoc($rq_classe_eleve))
{
	...
	while($row_rq_ue = mysql_fetch_assoc($rq_ue))
	{
		...
		while($row_rq_matiere = mysql_fetch_assoc($rq_matiere))
		{
			...
			mysql_select_db($database_connexion, $connexion);
			$query_rq_cc = sprintf("SELECT avg( NOTE.NOTE) as moy FROM NOTE, CONTROLE WHERE NOTE.IDCONTROLE=CONTROLE.IDCONTROLE AND CONTROLE.IDTYP_CONTROL=5 AND CONTROLE.IDMATIERE=%s AND NOTE.IDINDIVIDU=%s AND CONTROLE.IDPERIODE=%s AND VALIDER='oui'", GetSQLValueString($colname_rq_cc, "int"),GetSQLValueString($colname2_rq_cc, "int"),GetSQLValueString($colname3_rq_cc, "int"));
